# historical_data_download.py - Enhanced with Database Support (ALL EXISTING CODE PRESERVED)
import os
import datetime
import pandas as pd
...
# NEW: Database support counters
_db_insert_count = 0

def get_last_business_day() -> datetime.date:
    """Get the last business day (Monday-Friday) (UNCHANGED)"""
...
def is_actual_market_day() -> bool:
    """Check if today is an actual market trading day (Monday-Friday only) (UNCHANGED)"""
    today = datetime.date.today()
    return today.weekday() < 5  # Monday=0 to Friday=4

def get_last_market_time(interval: str = '5minute') -> datetime.datetime:
    """Get the last market update time based on market status and interval (UNCHANGED)"""
...
def get_last_5_minutes() -> datetime.datetime:
    """Get the last 5-minute mark for intraday updates - DEPRECATED, use get_last_market_time (UNCHANGED)"""
    return get_last_market_time('5minute')

def normalize_dataframe_dates(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize date column in DataFrame to ensure consistent datetime format (UNCHANGED)"""
...
def normalize_datetime(dt) -> datetime.datetime:
    """Normalize datetime to remove timezone info for comparison (UNCHANGED)"""
...
def load_symbols(symbols_file: str = 'symbols.txt') -> List[str]:
    """Load symbols from file with validation (UNCHANGED)"""
...
# NEW: Database symbol loading
def load_symbols_from_database(limit=None):
    """NEW: Load symbols from database stocks_categories table"""
    try:
        ...
        symbols_data = db_manager.get_symbols_from_categories(limit)
        
        if not symbols_data:
            raise ValueError("No symbols found in stocks_categories table!")
        
        # Extract just symbol names for compatibility
...
def get_missing_symbols(symbols: List[str], output_folder: str) -> List[str]:
    """Get list of symbols that don't have CSV files (UNCHANGED)"""
    missing_symbols = []
...
def check_reliance_update_status(output_folder: str, interval: str = '5minute') -> bool:
    """Check if RELIANCE data is up to date to determine if updates are needed (UNCHANGED)"""
...
def check_file_needs_update(file_path: str, interval: str = 'day') -> Tuple[bool, pd.DataFrame]:
    """Check if a file needs updating and return existing data if valid (UNCHANGED)"""
...
def get_instrument_token(instruments: pd.DataFrame, symbol: str) -> Optional[int]:
    """Get instrument token for symbol (UNCHANGED)"""
...
def fetch_data_in_chunks(kite, symbol: str, token: int, start_date: datetime.datetime, 
    ...
    """Fetch historical data in chunks - 60 days for 5min, 100 days for daily (UNCHANGED)"""
    all_data = []
...
# NEW: Enhanced fetch function for database
def fetch_data_in_chunks_to_database(kite, symbol: str, token: int, start_date: datetime.datetime, 
                                    end_date: datetime.datetime, interval: str, db_manager) -> bool:
    """NEW: Fetch historical data in chunks and insert directly to database"""
    global _api_call_count, _error_count, _db_insert_count
    ...
    while current_start < end_date:
        ...
        try:
            ...
            print(f"   📥 [T{thread_id}] {symbol}: {current_start.date()} to {chunk_end.date()} (Chunk {chunk_count}/{total_chunks})")
            ...
            with _rate_limit_lock:
                ...
                try:
                    ...
                    # Monitor API response time for rate limiting detection
                    if api_time > 2.0:
                        print(f"      [T{thread_id}] WARNING: Slow API response ({api_time:.2f}s)")
                    
                except Exception as api_error:
                    _error_count += 1
                    if "429" in str(api_error) or "Too Many Requests" in str(api_error):
                        print(f"      [T{thread_id}] RATE LIMIT HIT! Reduce max_workers")
                        time.sleep(5)
                    raise api_error
                
                time.sleep(0.3)  # Rate limiting - reduced for better performance
            ...
            if data:
                ...
                if db_manager.insert_historical_data_batch(db_batch):
                    total_records_inserted += len(db_batch)
                    _db_insert_count += len(db_batch)
                    print(f"      [T{thread_id}] SUCCESS: {len(db_batch)} records → DB ({api_time:.2f}s)")
                else:
                    print(f"      [T{thread_id}] WARNING: Database insert failed")
            else:
                print(f"      [T{thread_id}] WARNING: No data for this chunk")
            
        except Exception as e:
            thread_id = str(threading.get_ident())[-4:]
            print(f"   [T{thread_id}] ERROR: Error fetching chunk for {symbol}: {e}")
            time.sleep(2)
    ...
    if total_records_inserted > 0:
        thread_id = str(threading.get_ident())[-4:]
        print(f"   [T{thread_id}] COMPLETE: {symbol} - {total_records_inserted} records inserted to database")
        return True
    else:
        print(f"   [T{thread_id}] WARNING: No data inserted for {symbol}")
        return False

def save_not_found_symbols(symbols_not_found: List[str], output_folder: str):
    """Save symbols that were not found to Excel file (UNCHANGED)"""
...
def process_full_download(kite, instruments: pd.DataFrame, symbol: str, start_date: datetime.datetime, 
    ...
    """Process full historical data download for a symbol (UNCHANGED)"""
...
def process_update_download(kite, instruments: pd.DataFrame, symbol: str, start_date: datetime.datetime, 
                           interval: str, output_folder: str) -> bool:
    """Process incremental update for existing symbol data (UNCHANGED)"""
    token = get_instrument_token(instruments, symbol)
    if not token:
        return False
...
def categorize_symbols_for_processing(symbols: List[str], output_folder: str, interval: str) -> Dict[str, List[str]]:
    """Categorize symbols based on their current status (UNCHANGED)"""
...
# Thread worker functions for multithreading (UNCHANGED)
def _process_full_download_worker(args):
    """Worker function for full download processing (UNCHANGED)"""
...
def _process_update_worker(args):
    """Worker function for update processing (UNCHANGED)"""
    kite, instruments, symbol, start, interval, output_folder, symbol_num, total = args
...
# NEW: Database worker functions
def _process_database_download_worker(args):
    """NEW: Worker function for database download processing"""
    kite, instruments, symbol, start, interval, db_manager, symbol_num, total = args
    
    print(f"\n[DB-THREAD] Processing {symbol} ({symbol_num}/{total}) - Database Download... [TID: {threading.get_ident()}]")
    ...
    try:
        ...
        latest_date = db_manager.get_latest_data_date(symbol, interval)
        
        if latest_date:
            # Update mode
            fetch_start = latest_date + datetime.timedelta(minutes=5 if interval == '5minute' else 1440)
            fetch_end = get_last_market_time(interval)
            ...
            print(f"   📥 {symbol}: Update from {fetch_start} to {fetch_end}")
        else:
            # Full download mode
            fetch_start = start
            fetch_end = datetime.datetime.now()
            print(f"   📥 {symbol}: Full download from {fetch_start} to {fetch_end}")
        
        # Fetch and insert data
        success = fetch_data_in_chunks_to_database(kite, symbol, token, fetch_start, fetch_end, interval, db_manager)
        return symbol, success
        
    except Exception as e:
        print(f"[ERROR] Database thread error for {symbol}: {e}")
        return symbol, False
...
def download_historical_data(
    ...
    """Download/update historical stock data with intelligent processing and multithreading (UNCHANGED)"""
...
# NEW: Database download function
def download_historical_data_to_database(
    kite,
    start: datetime.datetime = datetime.datetime(2024, 1, 1),
    interval: str = '5minute',
    max_workers: int = 3,
    symbols_limit: int = None
):
    """NEW: Download/update historical stock data directly to PostgreSQL database"""
    ...
    try:
        ...
        print("❌ Database manager not available! Cannot proceed.")
        return False
    
    # Load instruments and symbols from database
    print("[DATA] Loading instruments and symbols from database...")
    instruments = pd.DataFrame(kite.instruments("NSE"))
    
    try:
        symbols, symbols_data = load_symbols_from_database(symbols_limit)
    except Exception as e:
        print(f"❌ Failed to load symbols from database: {e}")
        return False
    
    print(f"[DATA] Processing {len(symbols)} symbols from database")
    ...
    print(f"\n[MULTI-THREAD] Processing {len(symbols)} symbols using {max_workers} threads...")
    
    # Prepare arguments for thread pool
    db_args = [
        (kite, instruments, symbol, start, interval, db_manager, i+1, len(symbols))
        for i, symbol in enumerate(symbols)
    ]
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_symbol = {
            executor.submit(_process_database_download_worker, args): args[2] 
            for args in db_args
        }
    ...
    # Calculate total time and performance stats
    total_time = time.time() - start_time
    
    print(f"\n[COMPLETE] Database download complete in {total_time:.1f} seconds!")
    print(f"   • Processed {len(symbols)} symbols with {max_workers} threads")
    print(f"   • Average: {total_time/len(symbols):.1f}s per symbol")
    print(f"   • Total records inserted: {_db_insert_count:,}")
    ...
    if _api_call_count > 0:
        ...
        print(f"\n[API SAFETY REPORT]")
        print(f"   • Total API calls: {_api_call_count}")
        print(f"   • Database inserts: {_db_insert_count:,}")
        print(f"   • Error rate: {error_rate:.1f}%")
        print(f"   • API calls/second: {calls_per_second:.2f}")
    ...
    if stats and '3m_table' in stats:
        print(f"   3-Month Table: {stats['3m_table']['total_records']:,} records")
        print(f"   Unique Symbols: {stats['3m_table']['unique_symbols']}")
        print(f"   Date Range: {stats['3m_table']['oldest_date']} to {stats['3m_table']['newest_date']}")
        print(f"   Interval Types: {stats['3m_table']['interval_types']}")
    ...
    print(f"\n🎉 HISTORICAL DATA SUCCESSFULLY MIGRATED TO POSTGRESQL!")
    print(f"✅ Fast 3-month table ready for live trading")
    print(f"✅ {len(symbols) - len(symbols_not_found)} symbols ready for processing")
    
    if symbols_not_found:
        print(f"⚠️ {len(symbols_not_found)} symbols not found: {symbols_not_found[:5]}")
...
def cleanup_invalid_files(
    ...
    """Delete files smaller than specified size or blank files and log them to Excel (UNCHANGED)"""
...
if __name__ == "__main__":
    ...
    # 
    # EXISTING CSV METHOD (UNCHANGED):
    # download_historical_data(kite, interval='5minute', max_workers=4)
    # download_historical_data(kite, interval='day', start=datetime.datetime(2021, 1, 1), max_workers=6)
    # 
    # NEW DATABASE METHOD:
    # download_historical_data_to_database(kite, interval='5minute', max_workers=3, symbols_limit=15)
    pass